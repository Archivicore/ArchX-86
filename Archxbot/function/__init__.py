...
from Archxbot.utils import load_module
...
async def apk_dl(app_name, path, event):
    ...
    for link in result[:1]:
        ...
        for link in result:
            ...
            with open(f"{path}/{name}@Archxbot.apk", "wb") as f:
                f.write(r.content)
    await event.edit("`Apk, Downloaded. Let me Upload It here.`")
    final_path = f"{path}/{name}@fridayubkt.apk"
...
Music Downloaded And Uploaded By Archxbot
Get Your Archxbot From @fridayot"""
