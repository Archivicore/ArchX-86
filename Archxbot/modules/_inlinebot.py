...
from Archxbot import ALIVE_NAME, CMD_HELP, CMD_LIST, lang
from Archxbot.function import _deezer_dl, _ytdl
from Archxbot.modules import inlinestats
...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Archxbot"
...
if lang == "id":
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("Archxbot"):
            rev_text = query[::-1]
    ...
    @tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"jgnspam")))
    async def rip(event):
        if event.query.user_id == bot.uid:
    ...
    async def sed(event):
        ...
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""Archxbot Modul Tercantum Di Sini !\nUntuk Bantuan Lebih Lanjut tanyakan pada @Archivicore \n\nPlugins yang Dimuat Saat Ini: {len(CMD_LIST)}"""
    ...
    async def rip(event):
        ...
        await event.get_chat()
        him_id = event.query.user_id
        await eborg.send_message(
            entity=event.query.user_id,
            text="**Ini adalah Keamanan Pesan {DEFAULTUSER} untuk Menjauhkan Spam dan Memblokir Otomatis**\n\n`Protected by` [Archivicore](https//t.me/ArchivicoreOfficial)",
        ...
        if event.query.user_id == bot.uid and query.startswith("Archxbot"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "Repo Here", "https://github.com/Archivicore/ArchX-86"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/ArchivicoreOfficial")],
                ],
            )
            await event.answer([result])
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            sedok = "Who Are You , To USe This Get Your Own Archxbot ."
            await event.answer(sedok, cache_time=0, alert=True)
            return
        plugin_name = event.data_match.group(1).decode("UTF-8")
        if plugin_name in CMD_HELP:
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy Archxbot To Get Your Own Assistant, Repo Link [Here](https://github.com/Archivicore/ArchX-86)",
        )
...
async def inline_id_handler(event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="- Not Allowded -",
            text=f"You Can't Use This Bot. \nDeploy Archx To Get Your Own Assistant, Repo Link [Here](https://github.com/Archivicore/ArchX-86)",
        )
        await event.answer([resultm])
        return
    results = []
