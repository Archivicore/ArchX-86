...
async def _(event):
    ...
    packshortname = f"Archxbot_{userid}_Pack"
    await moods.edit("`Bas chori hone hi wala h🙃.....`")
    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@Archxbot.png"
    ...
    if is_a_s:
        ...
        packname = f"@{user.username} KangPack {pack}"
        packshortname = f"Archxbot_{userid}"  # format: Uni_Borg_userid
    else:
...
@Archx.on(Archx_on_cmd(pattern="getsticker ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
