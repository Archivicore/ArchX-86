...
import requests
from uniborg.util import Archx_on_cmd

from Archxbot import CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
...
@Archx.on(Archx_on_cmd(pattern="vt(?: |$)(.*)", outgoing=True))
@Archx.on(sudo_cmd(pattern="vt(?: |$)(.*)", allow_sudo=True))
async def download(target_file):
    Archx = await edit_or_reply(target_file, "`Processing ...`")
    if Config.VIRUSTOTAL_API_KEY is None:
        await Archx.edit(
            "Need to get an API key from https://virustotal.com\nModule stopping!"
        )
        return
    await Archx.edit("Processing using VirtualUserbot server ( ◜‿◝ )♡")
    ...
    if target_file.reply_to_msg_id:
        try:
            ...
            await Archx.edit(str(e))
        else:
            await Archx.edit(
                "Downloaded to `{}` successfully !!".format(downloaded_file_name)
            )
    else:
        await Archx.edit("Reply to a file..")
    ...
    try:
        ...
        await Archx.edit("your file is larger than 32 mb.")
    try:
        await Archx.edit(
        ...
        await Archx.edit("your file is larger than 32 mb.   --__--")
