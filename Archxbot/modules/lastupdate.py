...
from Archxbot.__init__ import Lastupdate
...
@Archx.on(Archx_on_cmd(pattern="latestupdate"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    ...
    await event.edit(f"🔰 Archxbot Has Been Restarted Or Updated {uptime} Ago !")
