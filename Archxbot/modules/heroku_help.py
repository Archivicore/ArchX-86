...
@Archx.on(Archx_on_cmd(pattern="usage$"))
...
@Archx.on(Archx_on_cmd(pattern="shp ?(.*)"))
async def lel(event):
...
@Archx.on(Archx_on_cmd(pattern="acolb (.*)"))
async def sf(event):
...
@Archx.on(Archx_on_cmd(pattern="tfa (.*)"))
async def l(event):
    hmm = event.pattern_match.group(1)
    app = Heroku.app(Var.HEROKU_APP_NAME)
...
@Archx.on(Archx_on_cmd(pattern="exit$"))
