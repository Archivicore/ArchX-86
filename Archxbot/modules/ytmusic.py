...
from Archxbot.function import progress
from Archxbot.function.FastTelethon import upload_file
from Archxbot.utils import edit_or_reply, Archx_on_cmd, sudo_cmd


@Archx.on(Archx_on_cmd(pattern="ytmusic ?(.*)"))
@Archx.on(sudo_cmd(pattern="ytmusic ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    Archxz = mio[0]["id"]
    thums = mio[0]["channel"]
    kekme = f"https://img.youtube.com/vi/{Archxz}/hqdefault.jpg"
...
@Archx.on(Archx_on_cmd(pattern="utubevid ?(.*)"))
@Archx.on(sudo_cmd(pattern="utubevid ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    urlissed = event.pattern_match.group(1)
