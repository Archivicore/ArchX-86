...
from ..utils import edit_or_reply, Archx_on_cmd, sudo_cmd


@Archx.on(Archx_on_cmd(pattern="app (.*)"))
@Archx.on(sudo_cmd(pattern="app (.*)", allow_sudo=True))
async def apk(event):
...
@Archx.on(Archx_on_cmd(pattern="appr (.*)"))
@Archx.on(sudo_cmd(pattern="appr (.*)", allow_sudo=True))
async def apkr(event):
    ...
    try:
        ...
        app_details += "\n\n<b>Download : </b> <a href='https://t.me/ArchivicoreOfficial'>Request_Here</a>"
        app_details += "\n\n===> @ArchivicoreOfficial <==="
        await event.edit(app_details, link_preview=True, parse_mode="HTML")
    except IndexError:
        await event.edit("No result found in search. Please enter **Valid app name**")
