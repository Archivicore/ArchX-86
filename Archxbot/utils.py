...
from var import Var
from Archxbot import CMD_LIST, LOAD_PLUG, SUDO_LIST, bot
from Archxbot.Configs import Config
from Archxbot.wraptools import (
    am_i_admin,
    ignore_bot,
    ignore_fwd,
    ignore_grp,
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import Archxbot.modules
        import Archxbot.utils

        path = Path(f"Archxbot/modules/{shortname}.py")
        name = "Archxbot.modules.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = Archxbot.utils
        sys.modules["Archx.util"] = Archxbot.utils
        sys.modules["userbot.utils"] = Archxbot.utils
        sys.modules["userbot.plugins"] = Archxbot.modules
        sys.modules["plugins"] = Archxbot.modules
        sys.modules["userbot"] = Archxbot
        mod.Config = Config
        ...
        sys.modules["Archxbot.events"] = Archxbot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Archxbot.modules." + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"Archxbot.modules.{shortname}"
...
def Archx_on_cmd(pattern=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
...
""" Userbot module for managing events.
 One of the main components of the Archxbot. """
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/ArchivicoreOffcial)"
            ...
            ftext += "--------BEGIN ARCHX USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"Archxbot/modules/assistant/{shortname}.py")
        name = "Archxbot.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules["Archxbot.modules.assistant" + shortname] = mod
